...
async function accountLogin(req, res) {
  ...
  try {
    ...
    req.flash(
      "notice",
      "Access forbidden. Please try again or contact the administrator.",
    );
...
async function buildUpdateAccountView(req, res, next) {
  ...
  if (
    !loggedInAccount ||
    (loggedInAccount.account_id != account_id &&
      loggedInAccount.account_type !== "Admin")
  ) {
...
async function updateAccount(req, res, next) {
  let nav = await utilities.getNav();
  let { account_firstname, account_lastname, account_email, account_id } =
    req.body;
  ...
  if (currentAccount && currentAccount.account_email !== account_email) {
    ...
    console.log('Account ID comparison - existing:', existingAccount?.account_id, 'current:', account_id, 'types:', typeof existingAccount?.account_id, typeof account_id);
